...
exports.getDrops = async (accessToken, placePk) => {
  try {
    ...
    logger.info(`All Drops of place Id ${placePk} retrieved : ${publicDrops.length + myDrops.length} in total`);
    return {
      writtenPlace,
      dropsCount,
      publicDrops,
...
exports.toggleDropLike = async (accessToken, dropPk) => {
  try {
    ...
    logger.error(`User ${user.nickname} failed to toggle likeComment Id ${commentPk} : ${error.message}`);
