...
exports.getDrops = async (req, res, next) => {
    try {
        ...
        const { writtenPlace, dropsCount, publicDrops, myDrops } = await dropServices.getDrops(
            accessToken,
            placePk,
        );
...
exports.getPublicDrops = async (req, res, next) => {
    try {
        const { placePk } = req.params;
        const accessToken = getAccess(req.headers);
        const { writtenPlace, dropsCount, publicDrops } = await dropServices.getPublicDrops(
            accessToken,
            placePk,
        );
...
exports.getMyDrops = async (req, res, next) => {
    try {
        const { placePk } = req.params;
        const accessToken = getAccess(req.headers);
        const { writtenPlace, dropsCount, myDrops } = await dropServices.getMyDrops(
            accessToken,
            placePk,
        );
